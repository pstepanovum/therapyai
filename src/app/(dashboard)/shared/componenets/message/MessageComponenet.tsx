...
import { Contact, Conversation, Message, MessagePageProps } from "./types";
...
export default function MessagePage({ userId, userType, onError }: MessagePageProps) {
  ...
  useEffect(() => {
    ...
    const fetchContacts = async () => {
      try {
        ...
        const sessionQuery = query(sessionsRef, where(`${userType}Id`, "==", userId));
        const sessionSnapshot = await getDocs(sessionQuery);
        const contactIds = new Set<string>();
        sessionSnapshot.forEach((doc) => {
          const otherPartyId = userType === "patient" ? doc.data().therapistId : doc.data().patientId;
          contactIds.add(otherPartyId);
        });

        const conversationsRef = collection(db, "conversations");
        const convoQuery = query(conversationsRef, where(`${userType}Id`, "==", userId));
          ...
          const contactsList = await Promise.all(
            Array.from(contactIds).map(async (contactId) => {
              ...
              const conversationId =
                userType === "patient" ? `${contactId}_${userId}` : `${userId}_${contactId}`;

              const conversation = snapshot.docs.find((d) => d.id === conversationId)?.data() as
                | Conversation
                | undefined;
  ...
  useEffect(() => {
    if (selectedContact && userId) {
      const db = getFirestore();
      const conversationId =
        userType === "patient" ? `${selectedContact}_${userId}` : `${userId}_${selectedContact}`;
  ...
  return (
    <Card className="h-screen w-full bg-white rounded-lg shadow-sm flex flex-col">
      <div className="flex flex-1 overflow-hidden">
        {/* Contacts Sidebar */}
        <div className="w-1/3 border-r border-[#F6F1F1] flex flex-col">
          <div className="p-4 border-b border-[#F6F1F1] flex-shrink-0">
          ...
          <div className="flex-1 overflow-y-auto">
            {contacts.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-[#146C94] p-4">
              ...
              <div className="divide-y divide-[#F6F1F1]">
                {contacts.map((contact) => (
                  <div
                    key={contact.id}
                    className={`
                      p-4 cursor-pointer transition duration-150 ease-in-out
                      ${
                    ...
                    <div className="flex items-start space-x-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          <p className="text-sm font-medium text-[#146C94] truncate">{contact.name}</p>
        ...
        <div className="w-2/3 flex flex-col">
          ...
          <div className="flex-1 overflow-hidden flex flex-col">
            {selectedContact ? (
                  ...
                  return (
                    <div key={index}>
                      ...
                      >
                        ...
                        <div
                          className={`max-w-[70%] ${msg.senderId === userId ? "items-end" : "items-start"}`}
                        >
                          <div
                            className={`
                              ...
                              ${
                                msg.senderId === userId
                                  ...
                                  : `${
                                      isNewMessage ? "bg-[#F6F1F1] border border-[#AFD3E2]" : "bg-[#F6F1F1]"
                                    } rounded-bl-sm`
                              }
              ...
              <div className="flex flex-col items-center justify-center flex-1 text-[#146C94]">
                <MessageSquare className="h-12 w-12 mb-4 opacity-50" />
